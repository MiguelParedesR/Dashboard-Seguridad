  ...
  (async () => {
    ...
    function syncLayout() {
      try {
        ...
        if (isDesktop() && sidebar.classList.contains('show') && !sidebar.classList.contains('collapsed')) {
          const w = sidebar.offsetWidth || 260;
          mainContainer.style.marginLeft = w + 'px';
          // ensure mobile overlay class removed
          document.body.classList.remove(BODY_CLASS_SIDEBAR_OPEN);
        } else {
          // default: clear margin
          mainContainer.style.marginLeft = '';
          // mobile overlay class active only when sidebar is open and not desktop
        ...
        // reflect collapsed state on body
