...
export default function LockersPage() {
  ...
  const states = useMemo(() => {
    ...
    const known = STATE_ORDER.filter((item) => dynamic.includes(item));
    const extras = dynamic.filter((item) => !STATE_ORDER.includes(item));
    return [...known, ...extras];
  }, [rows]);
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      const haystack = [row.codigo, row.local, row.area, row.colaborador_nombre || '']
        .join(' ')
        .toLowerCase();
    ...
    <main>
      <div className="page-head">
        <div>
          ...
          <h1>Vista general</h1>
          <p>Lectura consolidada de lockers activos, asignaciones vigentes e incidencias de llaves desde sus tablas canónicas.</p>
          {generatedAt ? (
            <p style={{ marginTop: 8, fontSize: 12 }}>
              Actualizado {formatDate(generatedAt)} {formatTime(generatedAt)}
            </p>
          ) : null}
        </div>
      ...
      <section className="section">
        <div className="section-head">
          <div>
            ...
            <p>{loading ? 'Cargando inventario…' : `${filtered.length} de ${rows.length} lockers visibles`}</p>
          </div>
          <div className="toolbar">
            <input
              className="input"
              aria-label="Buscar locker"
              placeholder="Buscar locker o colaborador"
              value={query}
              onChange={(event) => setQuery(event.target.value)}
            />
            <select className="select" aria-label="Filtrar por local" value={local} onChange={(event) => setLocal(event.target.value)}>
              <option value="TODOS">Todos los locales</option>
              {locals.map((item) => <option value={item} key={item}>{item}</option>)}
            </select>
            <select className="select" aria-label="Filtrar por área" value={area} onChange={(event) => setArea(event.target.value)}>
              <option value="TODAS">Todas las áreas</option>
              {areas.map((item) => <option value={item} key={item}>{item}</option>)}
            </select>
            <select className="select" aria-label="Filtrar por estado" value={estado} onChange={(event) => setEstado(event.target.value)}>
              <option value="TODOS">Todos los estados</option>
              {states.map((item) => <option value={item} key={item}>{stateLabel(item)}</option>)}
            </select>
              ...
              <tbody>
                {filtered.map((row) => (
                  <tr key={row.id}>
                    ...
                    <td>{row.local || '—'}</td>
                    <td>{row.area || '—'}</td>
                    <td><span className={`badge ${stateClass(row.estado)}`}>{stateLabel(row.estado)}</span></td>
                    <td>{row.colaborador_nombre || 'Sin asignación'}</td>
                    <td>{formatDate(row.fecha_asignacion)}</td>
                    <td>{row.llaves_esperadas === 1 ? '1 esperada' : `${row.llaves_esperadas} esperadas`}</td>
                    <td>
                      {row.incidencias_pendientes > 0 ? (
                        <span className="badge danger">
                          {row.incidencias_pendientes === 1 ? '1 pendiente' : `${row.incidencias_pendientes} pendientes`}
                        </span>
                      ) : '—'}
                    </td>
                  </tr>
